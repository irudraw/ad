...
<html lang="en">
  ...
  <body>
  	<div class="container">
  		...
  		<div class="right">
				<h1>Upgrading Premium URL Shortener to 5.0</h1> 
				<p>
					You are about to upgrade this software to version <strong>5.0</strong>. Please note that this will only update your database and NOT your files. It is strongly recommended that you first backup your database then your existing files in case something unexpected occurs. 
				</p>
				<p>
					Version 5.0 adds many new functionality including improvements in performance, features and security. For this reason, <strong>a lot of files</strong> were updated. <strong>Please read</strong> the update manual carefully in order to make sure the update is done as smoothly as possible. 
				</p>			
				<p>					
...
function get_query($dbinfo){
	...
	$query[]="ALTER TABLE `{$dbinfo["prefix"]}user` ADD `fbpixel` varchar(255) NULL";
	$query[]="ALTER TABLE `{$dbinfo["prefix"]}user` ADD `linkedinpixel` varchar(255) NULL";
	$query[]="ALTER TABLE `{$dbinfo["prefix"]}user` ADD `adwordspixel` varchar(255) NULL";
